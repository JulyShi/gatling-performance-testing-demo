...
class Ticket2 extends Simulation {
  ...
  val scn = scenario( "ticket" ).exec( ticketCode.ticketCode, login.login )


  //1分钟内从每秒10个用户增长到每秒200个用户
  setUp(
    scn.inject( rampUsersPerSec( 10 ) to 200 during (1 minutes) )
  ).protocols( httpConf )
}